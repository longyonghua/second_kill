...
var seckill = {
    ...
    handlerSeckill:function(seckillId,node){
        ...
        $.post(seckill.URL.exposer(seckillId),{},function(result){
            if(result && result['success']){
                ...
                if(exposer['exposed']){ //开启了秒杀
                    var md5 = exposer['md5'];
                    var killUrl = seckill.URL.execution(seckillId,md5);
                    console.log('killUrl:'+killUrl);
                    //使用one只绑定一次点击时间，防止重复点击多次
                    $('#killBtn').one('click',function(){
                        $(this).addClass('disable'); //先禁用按钮
                    ...
                    //客户端的计时时间和服务器的时间有偏差，客户端重新走一遍计时逻辑
                    var now = exposer['now'];
                    var start = exposer['start'];
                    var end = exposer['end'];
                    seckill.coundown(seckillId,now,start,end);
                }
            }else{
    ...
    countdown:function(seckillId,nowTime,startTime,endTime) {
        ...
        if (nowTime > endTime) { //秒杀结束
            ...
            }).on('finish.countdown',function(){ //时间倒计时完成后回调事件
                //获取秒杀地址，控制显示逻辑，执行秒杀
